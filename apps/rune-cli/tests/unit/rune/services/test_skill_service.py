...
def test_generate_tree_with_submodule(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "rune.repositories.git_repository.get_short_sha", mock_get_short_sha
    )
...
def test_update_skill_tree_with_ast_map(tmp_path):
    ...
    assert "### AST Map: `modules/foo`" in updated_content
    assert mock_repomap_content.strip() in updated_content
